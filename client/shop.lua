...
Citizen.CreateThread(function()
    while true do
        ...
        if IsInRegularShopZone(coords) or IsInPoliceShopZone(coords) then
...
RegisterNUICallback("TakeFromShop", function(data, cb)
    ...
    if type(data.number) == "number" and math.floor(data.number) == data.number then
        if string.find(data.item.name, "WEAPON_", 1) then
            data.item.type = 'item_weapon'
        end
        TriggerSeverEvent("suku:SellItemToPlayer", GetPlayerServerId(PlayerId()), data.item.type, data.item.name, tonumber(data.number))
    end
